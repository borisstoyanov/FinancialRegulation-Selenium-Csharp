...
{
    public class PO_RegulationsPopUp
    {
        public PO_RegulationsPopUp()
        {
            try
            {
        ...
        public PO_Regulations_CreateRegulationsPopUp CreateNewRegulations()
        {
            ClickCreate();
            return new PO_Regulations_CreateRegulationsPopUp();
        }
        ...
        public void VerifyRegulationsCreated()
        {
            Thread.Sleep(2000);
            Util.CheckIfOneOfTextsPresented("has been created", "has been requested");
        }

        public PO_Regulations_Edit EditRegulations()
        {
            SearchRegulations(Test.extRef);
            ClickEdit();
            return new PO_Regulations_Edit();
        }

        public void VerifyRegulationsEdited()
        {
            Thread.Sleep(2000);
        ...
        public void SearchRegulations(string regulationsName)
        {
            Thread.Sleep(2000);
            Browser.Browser.Instance.FindElement(By.Id("Search_NameSearch")).SendKeys(regulationsName);
            Browser.Browser.ClickByID("Search");
            Thread.Sleep(2000);
        }

        public PO_Regulations_Details ViewRegulationsDetails()
        {
            SearchRegulations(Test.extRef);
            ClickDetails();
            return new PO_Regulations_Details(Test.extRef);
        }
