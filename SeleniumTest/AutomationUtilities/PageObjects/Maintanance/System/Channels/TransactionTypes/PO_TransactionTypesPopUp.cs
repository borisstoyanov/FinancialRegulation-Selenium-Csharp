...
{
    public class PO_TransactionTypesPopUp
    {
        public PO_TransactionTypesPopUp()
        {
            try
            {
        ...
        public PO_TransactionTypes_CreateTransactionTypesPopUp CreateNewTransactionTypes()
        {
            ClickCreate();
            return new PO_TransactionTypes_CreateTransactionTypesPopUp();
        }
        ...
        public void VerifyTransactionTypesCreated()
        {
            Thread.Sleep(2000);
            Util.CheckIfOneOfTextsPresented("has been created", "has been requested");
        }

        public PO_TransactionTypes_Edit EditTransactionTypes()
        {
            SearchTransactionTypes(Test.extRef);
            ClickEdit();
            return new PO_TransactionTypes_Edit();
        }

        public void VerifyTransactionTypesEdited()
        {
            Thread.Sleep(2000);
        ...
        public void SearchTransactionTypes(string transactionTypesName)
        {
            Thread.Sleep(2000);
            Browser.Browser.Instance.FindElement(By.Id("Search_NameSearch")).SendKeys(transactionTypesName);
            Browser.Browser.ClickByID("Search");
            Thread.Sleep(2000);
        }

        public PO_TransactionTypes_Details ViewTransactionTypesDetails()
        {
            SearchTransactionTypes(Test.extRef);
            ClickDetails();
            return new PO_TransactionTypes_Details(Test.extRef);
        }
