...
{
    public class PO_AuditTradePopUp
    {
        public PO_AuditTradePopUp()
        {
            try
            {
                Util.WaitForElementVisibleByXPath("//a[contains(text(),'Trade Audit Entries')]", 60);
        ...
        internal static void CheckStaticIssue(string staticIssueText)
        {
            Util.WaitForElementVisibleByXPath("//a[contains(text(),'Trade Audit Entries')]", 60);
            Util.WaitForElementVisibleByXPath("//div[contains(text(),'entries')]", 120);

            Browser.Browser.ClickByXPath("//td[text()='"+Test.UTI+"']");
            Thread.Sleep(2000);
            Util.CheckIfTextPresented(staticIssueText);
        }
    }
}
