...
namespace AutomationUtilities.PageObjects.ReferenceData.InitiatorAggressor
{
    public class PO_InitiatorAggressor_Create
    {
        public PO_InitiatorAggressor_Create()
        {
            ...
            var nameElement = By.Id(ReferenceDataFieldIDs.Item_Name);
            Browser.Browser.Instance.FindElement(nameElement).SendKeys(Test.extRef);
            var fixedNameElement = By.Id(ReferenceDataFieldIDs.Item_Fixed_Name);
            Browser.Browser.Instance.FindElement(fixedNameElement).SendKeys(Test.extRef);
